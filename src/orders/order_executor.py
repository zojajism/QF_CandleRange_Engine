...
class OrderExecutionResult:
    ...
    lastTransactionID: Optional[int] = None  # OANDA's lastTransactionID from the response (for potential future use)
...
def sync_broker_orders(symbol: str) -> None:
    ...
    open_updates: List[Tuple] = []
    close_updates: List[Tuple] = []
    ...
    for (
        ...
        try:
            ...
            trade_root = (trade_detail.get("trade") or {})
            closing_ids = trade_root.get("closingTransactionIDs") or []
            close_transaction_id = str(closing_ids[0]) if closing_ids else None
            if close_transaction_id is None:
                ltx = trade_detail.get("lastTransactionID")
                close_transaction_id = str(ltx) if ltx is not None else None
        except Exception as e:
            logger.warning("[OrderSync] Failed to fetch trade %s from broker: %s",trade_id,e,)
            trade_detail, close_transaction_id = client.get_trade_close_transaction(trade_id)
            if close_transaction_id is not None:
                close_transaction_id = str(close_transaction_id)
            if close_transaction_id is None:
                continue

        # Normalize payload shape:
        # - get_trade(...) returns {"trade": {...}, ...}
        # - get_trade_close_transaction(...) can return {"transaction": {...}, ...}
        trade_obj = (trade_detail.get("trade") or trade_detail.get("transaction") or trade_detail)
        state = "CLOSED"
        ...
        realized_pl_str = (trade_obj.get("pl") or trade_obj.get("realizedPL"))
        ...
        if profit_ccy is not None and profit_ccy >= 0:
    ...
    # End of the for loop processing all signals -> apply bulk updates for all rows at once
    ...
    if open_updates:
        ...
        """
        with conn.cursor() as cur:
            cur.executemany(sql_update, open_updates)
        conn.commit()

    logger.info("[OrderSync] Finished broker sync, updated %d open trades.", len(open_updates))
    ...
    if close_updates:
        ...
        """
        with conn.cursor() as cur:
            cur.executemany(sql_update, close_updates)
        conn.commit()

    logger.info("[OrderSync] Finished broker sync, updated %d closed trades.", len(close_updates))

...
from db_general import fetch_sl_config
...
class SLManager:
    ...
    def get_trailing_sl_price(
        ...
        Returns the SL price, SL percentage, and whether the highest SL config level was reached.
        """
        if not self.configs:
        ...
        sorted_configs = sorted(self.configs, key=lambda x: x["distance_percentage"], reverse=True)
        ...
        last_level_reached = applicable_config == sorted_configs[0]
        ...
        return sl_price, applicable_config['sl_percentage'], last_level_reached

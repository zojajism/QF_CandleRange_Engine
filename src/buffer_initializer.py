...
def init_candle_buffer(exchange: str, symbols: List[str], timeframes: List[str]) -> None:
    """
...
def init_candle_buffer_Offset(exchange: str, symbols: List[str], timeframes: List[str]) -> None:
    ...
    # Candle capacity (per Keys)
    def candle_capacity_fn(key: Keys) -> int:
        return 300 - ps.Candle_Limit_offset 
    ...
    for symbol in symbols:
        for timeframe in timeframes:
            ...
            candles = get_candles_from_db_offset(key, candle_capacity_fn(key), ps.Candle_Limit_offset)
            for candle in candles:
                CANDLE_BUFFER.append(key, candle)

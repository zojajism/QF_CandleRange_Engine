...
class OpenSignal:
    ...
    sl_config_last_level: bool = False  # Whether the trailing SL has been applied to the last level of it
    actual_target_pips: Optional[Decimal] = None  
...
class OpenSignalRegistry:
        ...
        with self._lock:
            ...
            for row in rows:
                sig = OpenSignal(
                    ...
                    actual_target_pips=_to_decimal(row["actual_target_pips"]) if row.get("actual_target_pips") is not None else None,  # type: ignore
                )
    ...
    def _apply_trailing_sl(self, *, sig: OpenSignal, current_price: Decimal) -> None:
        ...
        if sig.sl_activated:
            return
        if sig.trailing_sl_price is None:
            return
    ...
    def _manage_trailing_sl(self, *, sig: OpenSignal, current_price: Decimal) -> None:
        ...
        if sig.actual_target_pips < ps.min_pip_for_TSL:
            return
        ...
        try:
            ...
            if _price_direction(price=current_price, target=sig.actual_tp_price, entry_price=sig.actual_entry_price, side=sig.side).lower() == "same":
                trailing_sl, sl_percentage, last_level_reached = calculate_trailing_sl(
            ...
            if trailing_sl is not None and ( (sig.trailing_sl_price is None) or (sig.side.lower() == "buy" and trailing_sl > sig.trailing_sl_price) or (sig.side.lower() == "sell" and trailing_sl < sig.trailing_sl_price)) :
                ...
                sig.sl_config_last_level = last_level_reached
    ...
    def _apply_fix_tp_sl(self, *, sig: OpenSignal, current_price: Decimal) -> None:
        
        if sig.tp_sl_activated:
            return

        try:
            if sig.side.lower() == "buy" and current_price >= sig.actual_tp_price:
            ...
            if sig.side.lower() == "sell" and current_price <= sig.actual_tp_price:
                logger.info(f"Applying Fix TP for {sig.side} signal: symbol:{sig.symbol}, trade_id:{sig.broker_trade_id}, tp_price:{sig.actual_tp_price}, current_price:{current_price}")
                sig.tp_sl_activated = True        
                close_position_by_trade_id(trade_id=sig.broker_trade_id)
    ...
    def _apply_fix_sl(self, *, sig: OpenSignal, current_price: Decimal) -> None:
        ...
        *,
        ...
        with self._lock:
            ...
            for sig in signals:
                ...
                self._apply_fix_tp_sl(sig=sig, current_price=price_to_check)
                
                # Apply fix SL if applicable
                #self._apply_fix_sl(sig=sig, current_price=price_to_check)
    ...
    def flush_distance_metrics(self, conn: psycopg.Connection) -> int:
        ...
        """
        to_flush: List[Tuple[OpenSignal, Decimal, Decimal, bool]] = []
            ...
            for _, signals in self._signals_by_symbol.items():
                for sig in signals:
                    ...
                    if sig.trailing_tp_price is None and sig.trailing_sl_price is None:
                        continue
                    to_flush.append((sig, sig.trailing_tp_price, sig.trailing_sl_price, sig.sl_config_last_level))
                    # Mark clean now; if DB fails, we'll mark dirty again on next ticks.
                    sig.dirty = False

        if not to_flush:
            logger.info(
                f"[open_signals] flush_trailing_prices: No updated trailing_prices for open signals"
            )            
            return 0
        ...
        try:
            with conn.cursor() as cur:
                for sig, trailing_tp_price, trailing_sl_price, sl_config_last_level in to_flush:
                    cur.execute(
                        sql,
                        (
                            trailing_tp_price,
                            trailing_sl_price,
                            sl_config_last_level,
                            sig.symbol,
                            sig.side,
                            sig.event_time,
                        ),
            ...
            logger.info(
                f"[open_signals] flush_trailing_prices: for {updated} open signals"
            )
